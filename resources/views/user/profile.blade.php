                    ...
                    <form action="{{ Auth::user()->profile ? route('profileUpdate') : route('postProfile') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="gender" id="gender" class="form-control color-gender">
                                ...
                                <option class="f-s color-gender my-3" value="Preferisco non specificarlo">Preferisco non
                                    specificarlo</option>
                        ...
                        <div class="mb-3 form-check">
                            <div class="col-12">
                                <input type="checkbox" class="form-check-input justify-content-end" id="contactMethod"
                                    name="contactMethod">
                                {{-- @if (Auth::user()->profile) @selected({{Auth::user()->profile->contactMetod}}) @endif> --}}
                                <label for="contactMethod" class="form-check-label f-s justify-content-start"> Voglio
                                    condividere il mio numero con gli utenti interessati ai miei annunci</label>
                            </div>
                        </div>
                        {{-- Da inserire select per metodo di contatto --}}
                        @if (Auth::user()->profile)
                            <button type="submit" class="btn ">Modifica i tuoi dati profilo</button>
                        @else
